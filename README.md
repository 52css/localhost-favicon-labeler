...
| 匹配 | hostname 或 `hostname:port` 匹配条件，不匹配路径 |
...
| 指定本地端口 | `127.0.0.1:8077, 127.0.0.1:8078, 127.0.0.1:14792` |
| 所有 dev 子域名 | `dev*` |
| 测试环境 | `test.example.com, staging.example.com` |

- 带端口的规则优先于普通 hostname 规则；同一级别内按从上到下的顺序命中。
- `[::1]` 匹配 IPv6 hostname，`[::1]:3000` 匹配指定 IPv6 端口。
